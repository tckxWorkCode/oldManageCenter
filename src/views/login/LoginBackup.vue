        ...
        <el-form class="form" ref="ruleForm" :model="ruleForm" :rules="rules" @submit.native.prevent>
          ...
          <el-form-item prop="passWord">
            ...
            <el-input :class="[this.inputType == 'password'?'inputStyle spacing':'inputStyle']" :type="inputType" placeholder="请输入密码" v-model="ruleForm.passWord"
              @paste.native.capture.prevent="handlePaste" @change="getFocus" @keyup.native="checkCapslock">
          ...
          <el-form-item prop="kaptchaReceived">
            <div class="label">验证码</div>
            <el-input class="inputStyle" type="text" placeholder="请输入验证码" v-model="ruleForm.kaptchaReceived"></el-input>
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        kaptchaReceived: [{ required: true, validator: validatePass, trigger: "blur" },],
      },
    };
  },
  methods: {
    submitForm(formName) {//登录
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          that.$store.dispatch("Login", that.ruleForm).then((res) => {
            if (res.status == 500) {
              that.refreshCode();
