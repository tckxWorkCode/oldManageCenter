<template>
  <el-dialog title="重置密码" top="30vh" width="30%" :visible.sync="dialogFormVisible" @close="diaClose" :close-on-click-modal="false">
    <el-form :label-position="labelPosition" label-width="100px" :rules="rules" :model="ruleForm" ref="ruleForm" size="small">
      <el-form-item prop="userId" label="用户名：">
        <el-input style="width:90%" v-model="userId" disabled ></el-input>
      </el-form-item>
...
export default {
  ...
  methods: {
    diaOpen () {
      ...
      that.userId = localStorage.getItem('usershowName')
    },
    diaClose () {
      var that = this
      that.dialogFormVisible = false
      that.$emit('diaVisible')
    },
    pwex0() {
      var that = this;
      ...
      that.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          request({
            ...
            if (res.status === 0) {
              that.$message({message: '操作成功!', type: 'success', duration: 0, showClose: true })
              that.dialogFormVisible = false
              that.$emit('successup')
            } else {
              that.btnvalue1 = false
    ...
    keyDown(e) {
      if (e.keyCode ==13 && this.dialogFormVisible == true) {this.submitForm('ruleForm')}
    },
  }
}
</script>
