<template>
  <el-dialog title="迁入文件" width="30%" :visible.sync="dialogFormVisible" @closed="diaclose" :close-on-click-modal="false" :append-to-body="true">
    <el-form :label-position="labelPosition" label-width="125px" :rules="rules" :model="ruleForm" ref="ruleForm" size="small">
      <el-form-item label="选择文件：" prop="file">
        <el-input class="fileLabel" style="float: left" type="text" v-model="ruleForm.file" placeholder="请选择文件" :disabled="true"></el-input>
        <el-upload class="uploadc" :http-request="uploadFile" :before-upload="beforeUpload"
          :show-file-list="false" name="file" ref="upload" :action="url" :file-list="fileList"
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button size="small" class="sureBtn" v-if="btnf1" @click="submit('ruleForm')">确定</el-button>
    </div>
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    uploadFile(content) {
      ...
      form.append("tree_path",that.path)
      form.append("node_id",that.nodeId)
      form.append("tree_path_id",that.id)
      request({
        url: that.url,
        method: "post",
        data: form,
        ...
        if (res.status === 0) {
          ...
          that.uploadmessage = "";
          that.$message({ message: "迁入成功!", type: "success", showClose: true, });
      ...
      that.$refs[formName].validate((valid) => {
        if (valid) {
          if (that.uploadmessage === "") {
            ...
            that.btnf1 = true;
            // that.$message({ message: that.uploadmessage, type: "error", duration: 0,showClose: true,});
          }
        } else {
          that.btnf1 = true;
          return false;
