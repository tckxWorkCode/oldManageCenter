<template>
  <el-dialog title="信息采集" top="10vh" width="35%" :visible.sync="dialogFormVisible" @close="diaClose" :close-on-click-modal="false">
    <el-form :label-position="labelPosition" label-width="100px" :rules="rules" :model="ruleForm" ref="ruleForm" size="small">
      <div class="itemw">
        <el-form-item class="uploadItem" prop="name" label="名称：">
          <el-input style="width:80%" v-model="ruleForm.name" clearable placeholder='请输入名称'></el-input>
        </el-form-item>
        <el-form-item class="uploadItem" prop="path" label="程序路径：">
          <el-input style="width:80%" id="path" v-model="ruleForm.path" clearable placeholder='请输入程序路径'></el-input>     
        </el-form-item>
      </div>
      <div class="itemw">
    ...
    <div slot="footer" class="dialog-footer">
      <el-col :span="16">
        <div class="tipText">信息采集完成后需手动添加配置才可生效</div>
      </el-col>
  ...
  data () {
    return {
      ...
      rules: {
        ...
        path: [{ required: true, message: '请输入程序路径', trigger: 'blur' },],
    ...
    submitForm (formName) {
      ...
      that.$refs[formName].validate((valid) => {
        if (valid) {
          that.btnvalue1 = true
          request({
            url: '/sys/informationMonitoring',
            data: {
              node_id:that.terminalSelection.nodeId,
              name:that.ruleForm.name,
              path:that.ruleForm.path,
            },
            method: 'post'
          }).then((response) => {
            var res = response.data
