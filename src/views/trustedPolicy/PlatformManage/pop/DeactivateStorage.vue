<template>
  <!--新建开始-->
  <el-dialog title="停用" width="23%" :visible.sync="dialogFormVisible" @closed="diaclose" :close-on-click-modal="false" class="deactivate">
    <div style="margin-bottom:10px"><i class="el-icon-warning" style="color:#E6A23C;font-size:24px;vertical-align: middle;margin-right:16px"></i>确定停用此存储区域？请输入tcm密码：</div>
    <el-form :rules="rules" :model="ruleForm" ref="ruleForm" @submit.native.prevent>
      <el-form-item prop="tcmPassword">
        <el-input style="width:99%" :type="password0" v-model="ruleForm.tcmPassword" placeholder="请输入tcm密码，密码不允许输入空格" clearable :onkeyup="ruleForm.tcmPassword=ruleForm.tcmPassword.replace(/[ ]/g,'')">
  ...
  methods: {
    submitForm(formName) {
      ...
      that.$refs[formName].validate((valid) => {
        if (valid) {
          request({url: "/sec/cancelStorage?id=" + this.rowParam.id +"&password=" +this.ruleForm.tcmPassword,method: "post",}).
            then((response) => {
