<template>
  <div class="main">
    <div class="container">
      <div class="topNav">
        <el-breadcrumb class="breadNav" separator="/">
          ...
          <el-breadcrumb-item class="crumb2">密码管理</el-breadcrumb-item>
        </el-breadcrumb>
      ...
      <div class="wrap">
        ...
        <div class="mainContainer">
          ...
          <el-table :data="strategyData" size="small" ref="strategyTable" tooltip-effect="light" @sort-change='sortChange'
           ...
           :row-class-name="tableRowClassName" :row-style="selectedstyle" @row-click="rowClick">
            <template slot="empty">
              <div>暂无数据</div>
            </template>
            ...
            <el-table-column prop="tcmStatus" label="密码修改状态" :formatter="statusVal"></el-table-column>
            <el-table-column prop="tcmDate" show-overflow-tooltip min-width="90%" label="创建时间" sortable='custom'></el-table-column>
    ...
    <edit-password ref="poplay" @editSuccess="initData"></edit-password>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {  
    loadData() {
      var that = this
      this.loading = true
      request({url: "/sys/tcmPwdList",method: "post",
        data: {
        ...
        if(res.list) {
          res.list.map(function(value,index) {
            value.tcmDate = timeToZh(value.tcmDate);
          })
    ...
    statusVal(row) {
      var option = ""
      if (row.tcmStatus == 0) {
        option =<span class="wathet">修改中</span>
      } else if (row.tcmStatus == 1) {
        option =<span class="green">已修改</span>
      } else if (row.tcmStatus == 2) {
        var err =  errorValue(row.tcmRes)
        if (err) { option = <span class="red">修改失败（{err}）</span>}
        else if(row.tcmRes) { option = <span class="red">修改失败（错误码：{row.tcmRes}）</span>}
        else { option = <span class="red">修改失败</span>}
      } else {
