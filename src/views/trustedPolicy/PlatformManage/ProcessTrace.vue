<template>
  <div class="main">
    <div class="container">
      <div class="topNav">
        <el-breadcrumb class="breadNav" separator="/">
          ...
          <el-breadcrumb-item class="crumb2">进程防跟踪</el-breadcrumb-item>
        </el-breadcrumb>
        <!-- <div class="title">进程防跟踪</div> -->
      </div>
      <div class="wrap">
        <div class="search">
        ...
        <div class="mainContainer">
          <div class="tableTitle">进程防跟踪</div>
          <el-table :data="strategyData" size="small" ref="strategyTable" tooltip-effect="light"  @sort-change='sortChange'
           ...
           :cell-style="{padding:'0px',color: 'rgba(0, 0, 0, 0.9)'}" style="width: 99.9%;font-size: 14px"   :fit='true'>
            ...
            <el-table-column prop="arch" label="体系架构"></el-table-column>
            <el-table-column prop="process_tracer_status" label="进程跟踪状态" :formatter="statusVal"></el-table-column>
            <!-- <el-table-column prop="lpolicyStatus.processTracerDate" :show-overflow-tooltip="true" min-width="90%" label="创建时间" sortable='custom'></el-table-column> -->
            <el-table-column prop="up_date" show-overflow-tooltip min-width="90%" label="创建时间" sortable='custom'></el-table-column>
    ...
    <config-process-trace ref="poplay" @success="initData"></config-process-trace>
  </div>
</template>
<script>
...
import ConfigProcessTrace from './pop/ConfigProcessTrace.vue'
export default {
  name:"ProcessTrace",
  components:{ConfigProcessTrace},
  data() {
    return {
      loading:false,
      formInline:{
  ...
  methods: {  
    loadData() {
      var that = this
      this.loading = true
      request({url: "/lProcessTracerList",method: "post",
        data: {
        ...
        if(res.list) {
          res.list.map(function(value,index) {
            if(value.lpolicyStatus) {
              value.lpolicyStatus.processTracerDate = timeToZh(value.lpolicyStatus.processTracerDate);
            }
          })
    ...
    statusVal(row) {
      var option = ""
      if (row.process_tracer_status == 0) {
        option =<span class="wathet">下发中</span>
      } else if (row.process_tracer_status == 1) {
        option =<span class="green">已下发</span>
      } else if (row.process_tracer_status == 2) {
        var err =  errorValue(row.process_tracer_res)
        if (err) { option = <span class="red">下发失败（{err}）</span>}
        else if(row.process_tracer_res) { option = <span class="red">下发失败（错误码：{row.process_tracer_res}）</span>}
