    ...
    <el-form :label-position="labelPosition" label-width="100px" :rules="rules" :model="ruleForm" ref="ruleForm" size="small">
      <div class="itemw">
        <el-form-item class="uploadItem" prop="path" label="文件路径：">
          <el-input style="width:70%" id="path" v-model="ruleForm.path" clearable placeholder='请输入文件全路径' @change="handlePath"></el-input>     
        </el-form-item>
      </div>
      <div class="itemw">
  ...
  methods: {
    ...
    submitForm (formName) {
      ...
      that.$refs[formName].validate((valid) => {
        if (valid) {
          that.btnvalue1 = true
          request({ url: '/sec/collectFileInfo', data: formData, method: 'post'
          }).then((response) => {
            var res = response.data
            if (res.status === 0) {
