<template>
  <div class="main">
    <div class="container">
      <div class="topNav">
        <el-breadcrumb class="breadNav" separator="/">
          ...
          <el-breadcrumb-item class="crumb2">关键文件度量</el-breadcrumb-item>
        </el-breadcrumb>
        <!-- <div class="title">关键文件度量</div> -->
      </div>
      <div class="wrap">
        <div class="search">
        ...
        <div class="mainContainer">
          <div class="tableTitle">关键文件度量列表</div>
          <div class="popBtn">
            <el-button class="operateBtn" size="small" @click="addIdentity">信息采集</el-button>
          </div>
          <el-table :data="keyfileData" size="small" ref="identityTable" tooltip-effect="light" 
            ...
            <el-table-column prop="nodeIp" label="网络地址" min-width="90%">
            ...
            <el-table-column prop="hash" label="HASH" min-width="180%" show-overflow-tooltip></el-table-column>
            <el-table-column prop="status" label="状态" min-width="110%" :formatter="statusVal"></el-table-column>
            <el-table-column prop="createTime" label="创建时间" min-width="100%" sortable='custom' show-overflow-tooltip></el-table-column>
            <el-table-column prop="collectTime" label="采集时间" min-width="100%" sortable='custom' show-overflow-tooltip></el-table-column>
            <el-table-column fixed="right" label="操作" align="left" min-width="130%">
              <template slot-scope="props">
                <el-button style="color:#1ac492" v-if="props.row.status == '1'" @click="configure(props.row)" type="text">配置</el-button>
                <el-button type="text" v-else disabled>配置</el-button>
                <el-button style="color:#1ac492" v-if="props.row.status == '2'" @click="reacquisition(props.row)" type="text">重新采集</el-button>
    ...
    <div class="dialogContent" v-if="showDialog == true">
      <document-details ref="details" :rowMessage="rowMessage" @closeDialog="closeDialog"></document-details>
    </div>
    <div v-if="mask==true" class="mask" @click="closeDialog"></div> 
    <!-- 新建弹出框    -->
    <info-acquisition ref="poplay" @addSuccess="addSuccess"></info-acquisition>
  </div>
</template>
<script>
...
import InfoAcquisition from './pop/InfoAcquisition.vue';
import DocumentDetails from './pop/DocumentDetails.vue';
export default {
  name:"KeyDocumentMetrics",
  components: { InfoAcquisition,DocumentDetails},
  data() {
    return {
      loading:false,
      formInline:{
  ...
  methods: {  
    loadData() {
      var that = this
      this.loading = true
      request({url: "/sec/keyFilePolicySearch",method: "post",
        data: {
        ...
        if(res.list) {
          res.list.map(function(value,index) {
            ...
            value.collectTime = timeToZh(value.collectTime);
          })
          that.keyfileData = res.list
    ...
    statusVal(row) {
      ...
      if (row.status == 0) {
        ...
        else if(row.res) { option = <span class="red">采集失败（错误码：{row.res}）</span>}
    ...
    del(row){
      ...
      this.$confirm("此操作将永久删除该数据，是否继续？","提示", {
        ...
        request({url: "/sec/delKeyFilePolicy?id="+row.id,method: "post",
        }).then((response) => {
          var res = response.data
          // console.log(res)
          if(res.status==0) {
    ...
    configure(row) {
      // console.log(row)
      this.$confirm("是否配置此策略？","提示", {
        confirmButtonText:"确定",
        cancelButtonText:"取消",
        type:"warning"
      }).then(() => {
        request({url: "/sec/setKeyFilePolicy?id="+row.id,method: "post",
        }).then((response) => {
          var res = response.data
          // console.log(res)
          if(res.status==0) {
    ...
    reacquisition(row) {
      ...
      formData.append("node_id", row.nodeId);
      ...
      this.$confirm("是否重新采集信息？","提示", {
        ...
        request({url: "/sec/collectFileInfo",method: "post", data: formData,
        }).then((response) => {
          var res = response.data
          // console.log(res)
          if(res.status==0) {
