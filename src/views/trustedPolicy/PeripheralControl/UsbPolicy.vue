<template>
  <div class="main">
    <div class="container">
      <div class="topNav">
        <el-breadcrumb class="breadNav" separator="/">
          ...
          <el-breadcrumb-item class="crumb2">U盘策略</el-breadcrumb-item>
        </el-breadcrumb>
        <!-- <div class="title">U盘策略</div> -->
      </div>
      <div class="wrap">
        <div class="search">
        ...
        <div class="mainContainer">
          ...
          <el-table :data="terminalData" ref="terminalTable" tooltip-effect="light" @sort-change='sortChange'
            ...
            <el-table-column prop="udiskStatus" label="状态" min-width="100%" :formatter="statusVal"></el-table-column>
            ...
            <el-table-column prop="udiskTime" :show-overflow-tooltip="true" min-width="100%" label="创建时间" sortable='custom'></el-table-column>
            <el-table-column fixed="right" label="操作" align="left" min-width="130%">
              <template slot-scope="scope">
                <el-button style="color:#1ac492" type="text" @click="configUsb(scope.row)">配置</el-button>
                <el-button style="color:#1ac492" type="text" @click="checkOnline(scope.row)">查看在线设备</el-button>
              </template>
            </el-table-column>
          </el-table>
    ...
    <!-- 新建弹出框    -->
    <config-usb ref="usb" @success="initData"></config-usb>
    <check-online ref="online"></check-online>
  </div>
</template>
<script>
...
import ConfigUsb from "./pop/ConfigUsb.vue"
import CheckOnline from "./pop/CheckOnline.vue"
export default {
  name:"UsbPolicy",
  components: { ConfigUsb,CheckOnline},
  data() {
    return {
      loading:false,
      formInline:{
  ...
  methods: {  
    ...
    statusVal(row) {
      var option = ""
      if (row.udiskStatus == 0) {
        option =<span class="wathet">配置中</span>
      } else if (row.udiskStatus == 1) {
        option =<span class="green">已配置</span>
      } else if (row.udiskStatus == 2) {
        var err =  errorValue(row.udiskRes)
        if (err) { option = <span class="red">配置失败（{err}）</span>}
        else if(row.udiskRes) { option = <span class="red">配置失败（错误码：{row.udiskRes}）</span>}
        else { option = <span class="red">配置失败</span>}
      } else {
