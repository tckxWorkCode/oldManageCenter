    ...
    <div class="container">
      <div class="topNav">
        <el-breadcrumb class="breadNav" separator="/">
          ...
          <el-breadcrumb-item class="crumb2">白名单度量</el-breadcrumb-item>
        </el-breadcrumb>
        <!-- <div class="title">白名单度量</div> -->
      </div>
      <div class="wrap">
        <div class="search">
        ...
        <div class="mainContainer">
          <div class="tableTitle">白名单度量列表</div>
          <el-table :data="policyData" size="small" ref="policyTable" tooltip-effect="light"  @sort-change='sortChange'
            ...
            <el-table-column prop="smeasureAuthStatus" label="白名单度量策略状态" min-width="100%" :formatter="statusVal"></el-table-column>
  ...
  methods: {
    ...
    statusVal(row) {
      ...
      if (row.lpolicyStatus == null) {
        option = ""
      } else if (row.lpolicyStatus.smeasureAuthStatus == 0) {
        option =<span class="wathet">已下发</span>
      } else if (row.lpolicyStatus.smeasureAuthStatus == 1) {
        option =<span class="green">已生效</span>
      } else if (row.lpolicyStatus.smeasureAuthStatus == 2) {
        var err =  errorValue(row.lpolicyStatus.smeasureAuthRes)
        if (err) { option = <span class="red">下发失败（{err}）</span>}
        else if(row.lpolicyStatus.smeasureAuthRes) { option = <span class="red">下发失败（错误码：{row.lpolicyStatus.smeasureAuthRes}）</span>}
        else { option = <span class="red">下发失败</span>}
      } else if (row.lpolicyStatus.smeasureAuthStatus == 3) {
        option =<span class="wathet">注销中</span>
      } else if (row.lpolicyStatus.smeasureAuthStatus == 4) {
        option =<span class="green">已注销</span>
      } else if (row.lpolicyStatus.smeasureAuthStatus == 5) {
        var err =  errorValue(row.lpolicyStatus.smeasureAuthRes)
        if (err) { option = <span class="red">注销失败（{err}）</span>}
        else if(row.lpolicyStatus.smeasureAuthRes) { option = <span class="red">注销失败（错误码：{row.lpolicyStatus.smeasureAuthRes}）</span>}
    ...
    del(row){
      ...
      this.$confirm("此操作将永久删除该数据，是否继续？","提示", {
        ...
        request({url: "/sys/disablePolicyAuth",method: "post",
          data:{ node_id: row.nodeId, policy_type:2}
        }).then((response) => {
          var res = response.data
          // console.log(res)
          if(res.status==0) {
