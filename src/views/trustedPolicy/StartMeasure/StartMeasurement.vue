<template>
  <div class="main">
      ...
      <div class="wrap">
        ...
        <div class="mainContainer">
          ...
          <el-table :data="startMeasureData" size="mini" ref="startMeasureTable" tooltip-effect="light" 
            ...
            <el-table-column prop="installerTime" label="时间" min-width="100%" sortable='custom' show-overflow-tooltip></el-table-column>
            <el-table-column fixed="right" label="操作" min-width="100%">
              <template slot-scope="scope">
                <el-button type="text" style="color:#1ac492" @click="measureRecord(scope.row)">度量记录</el-button>
                <!-- <el-button v-if="scope.row.onlineStatus == 0" style="color:#D9D9D9" type="text" :id="scope.row.nodeId" @click="openConfig(scope.row)">配置策略</el-button>
                <el-button v-else style="color:#1ac492" type="text" :id="scope.row.nodeId" @click="openConfig(scope.row)">配置策略</el-button>
                <el-button type="text" style="color:#1ac492" @click="policyRecord(scope.row)">策略记录</el-button> -->
                <!-- <el-button type="text" v-if ="scope.row.lpolicyStatus.bmeasureStatus == '2'" @click="reissue(scope.row)">重新下发</el-button> -->
              </template>
            </el-table-column>
          </el-table>
    ...
    <!-- 导入文件弹出框    -->
    <!-- <config-pop ref="poplay" @handleSuccess="refreshData"></config-pop> -->
    <!-- <policy-record ref="record"></policy-record> -->
    <measure-record ref="record" @handleSuccess="refreshData"></measure-record>
  </div>
</template>
<script>
...
import ConfigPop from './pop/ConfigPop'
import MeasureDetails from './pop/MeasureDetails.vue'
import PolicyRecord from './pop/PolicyRecord.vue'
import MeasureRecord from './pop/MeasureRecord.vue';
export default {
  name:"StartMeasurement",
  components: { ConfigPop,MeasureDetails,PolicyRecord, MeasureRecord },
  data() {
    return {
      loading:false,
      formInline:{
