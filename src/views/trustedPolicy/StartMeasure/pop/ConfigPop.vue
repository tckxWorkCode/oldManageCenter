<template>
  <el-dialog title="启动度量配置" top="5vh" width="65%" :visible.sync="dialogFormVisible" @close="diaClose" :close-on-click-modal="false">
    <el-button size="small" @click="openPolicy">获取策略记录</el-button>
...
import PolicyRecord from './PolicyRecord.vue'

export default {
  name: 'ConfigPop',
  components: { AddPolicy,PolicyRecord },
  data () {
    return {
  ...
  methods: {
    ...
    openPolicy() {
      this.$refs.record.diaOpen(this.rowData)
    },
    submit() {
      var that = this
      var newArr = []
    ...
    updateData(data) {
      data.pushed = true
      data.show = false
      data.num = this.tableData.length
