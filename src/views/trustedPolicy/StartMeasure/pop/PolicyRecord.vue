<template>
  <el-dialog title="启动度量记录" top="5vh" width="55%" :visible.sync="dialogFormVisible" @close="diaClose" :close-on-click-modal="false" append-to-body>
    <el-form label-position="right" label-width="90px" ref="ruleForm" size="small">
    ...
    <div class="statusTip">
      ...
      &nbsp;&nbsp; <span v-show="this.getOldBtn == true"><el-button size="small" @click="getOldList">获取历史记录</el-button></span>
    </div>
    ...
    :header-cell-style="{height: '48px','background-color': '#F0F2F5',color: 'rgba(0, 0, 0, 0.72)',padding:'0px'}" max-height="55vh">
      ...
      <el-table-column prop="result" label="度量结果" min-width="100%">
        <template slot-scope="props">
          <span v-if="props.row.result =='0'">可信</span>
          <span v-else-if="props.row.result =='1'">不可信</span>
          <span v-else-if="props.row.result =='2'">未度量</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    diaOpen (param) {
      ...
      that.status = "获取配置中，请稍后"
      request({url: "/sec/lBMeasureRecord",method: "post", data:{node_id:param.nodeId}})
      .then((response) => {
      ...
      setTimeout(function() {
        ...
        if (JSON.stringify(that.tableData) =='[]' || JSON.stringify(that.tableData) == undefined) {
          ...
          that.status = "请求已超时，请点击右侧按钮获取历史记录"
          that.getOldBtn = true
        } else { 
