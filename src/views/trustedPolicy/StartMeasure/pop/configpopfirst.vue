...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      //   arr1 = newArr
      // }
      // if (JSON.stringify(this.expandData)!= '[]' &&typeof(that.expandData)!= 'undefined') {
      //   arr2 = arr1.concat(this.expandData)
      // } else {
      //   arr2 = arr1
      // }
      // console.log(arr2)
    ...
    expandChange(row,expandedRows) {
      ...
      // } else {
      //   that.expandData = that.expandData
      // }
    ...
    addDomain(index,row) {
      ...
      // } else 
      if( index != this.expands.toString()) {
        this.$message.warning("请展开要添加的项");
      } else {
        // console.log(this.expandData)
        if (this.expands.length==0) {
          this.expands = [index]
