    ...
    <div class="container">
      <div class="topNav">
        <el-breadcrumb class="breadNav" separator="/">
          ...
          <el-breadcrumb-item class="crumb2">文件访问控制日志</el-breadcrumb-item>
        </el-breadcrumb>
        <!-- <div class="title">文件访问控制日志</div> -->
      </div>
      <div class="wrap">
        <div class="search">
        ...
        <div class="mainContainer">
          ...
          <div class="popBtn">
            <el-button class="operateBtn" size="small" @click="exportData" :disabled="fileControlLogData.length==0">导出</el-button>
          </div>
          <el-table :data="fileControlLogData" size="small" ref="whiteListLogTable"  @sort-change='sortChange'
           ...
           :cell-style="{color: 'rgba(0, 0, 0, 0.9)'}" tooltip-effect="light" style="width: 99.9%;font-size: 14px"   :fit='true'>
            ...
            <el-table-column prop="user" label="用户名" min-width="70%"></el-table-column>
            ...
            <el-table-column prop="object" label="客体" min-width="130%"></el-table-column>
            <el-table-column prop="hash" label="主体Hash" min-width="130%">
              <template slot-scope="props">
            ...
            <el-table-column prop="operate" label="操作" min-width="60%">
              <template slot-scope="props">
                <span v-if="props.row.operate == '1'">执行</span>
                <span v-else-if="props.row.operate == '2'">打开</span>
                <span v-else-if="props.row.operate == '4'">写</span>
                <span v-else-if="props.row.operate == '8'">读</span>
                <span v-else-if="props.row.operate == '16'">追加</span>
                <span v-else-if="props.row.operate == '32'">删除</span>
                <span v-else-if="props.row.operate == '64'">创建</span>
                <span v-else-if="props.row.operate == '128'">重命名</span>
              </template>
            </el-table-column>
            <el-table-column prop="result" label="结果" min-width="60%">
              <template slot-scope="props">
                <span v-if="props.row.result == '1'">成功</span>
                <span v-else-if="props.row.result == '2'">失败</span>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {  
    loadData() {
      var that = this
      this.loading = true
      request({url: "/aud/log/fileLog",method: "post",
        data: {
        ...
        if (res.list) {
          ...
          that.fileControlLogData = res.list
        }
    ...
    exportData() {
      ...
      this.$confirm("确定导出此数据？","提示", {
        ...
        request({url: "/aud/downloadLinuxFileAccess",method: "post",responseType: "blob", 
          data: form
        }).then((response) => {
          var res = response.data
          // console.log(res)
