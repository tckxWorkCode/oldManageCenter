    ...
    <div class="container">
      <div class="topNav">
        <el-breadcrumb class="breadNav" separator="/">
          ...
          <el-breadcrumb-item class="crumb2">告警日志</el-breadcrumb-item>
        </el-breadcrumb>
      ...
      <div class="wrap">
        <div class="search">
          <el-form :inline="true" label-position="right" label-width="100px" :model="formInline" ref="formInline" class="formInline" size="small" @submit.native.prevent>
            ...
            <el-form-item label="系统类型：" prop="systemType" v-show="advancedSearch==true">
              <el-select v-model="formInline.systemType" clearable placeholder="请选择" style="width:180px">
            ...
            <el-form-item label="告警类型：" prop="policyType" v-show="advancedSearch==true">
              <el-select v-model="formInline.policyType" clearable placeholder="请选择" style="width:180px">
        ...
        <div class="mainContainer">
          ...
          <!-- <div class="popBtn">
            <el-button class="operateBtn" size="small" @click="exportData">导出</el-button>
          </div> -->
          <el-table :data="dynamicLogData" size="small" ref="dynamicLogTable" tooltip-effect="light" style="width: 99.9%;font-size: 14px" 
           ...
           :cell-style="{color: 'rgba(0, 0, 0, 0.9)'}" @sort-change='sortChange'   :fit='true'>
            ...
            <el-table-column prop="warningType" label="告警类型" min-width="80%">
              <template slot-scope="props">
                <span v-if="props.row.warningType == '1'">白名单基准库校验失败</span>
                <span v-else-if="props.row.warningType == '2'">关键文件基准库校验失败</span>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {  
    loadData() {
      var that = this
      this.loading = true
      request({url: "/aud/log/warningLog",method: "post",
        data: {
