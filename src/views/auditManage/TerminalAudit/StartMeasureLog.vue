    ...
    <div class="container">
      <div class="topNav">
        <el-breadcrumb class="breadNav" separator="/">
          ...
          <el-breadcrumb-item class="crumb2">启动度量日志</el-breadcrumb-item>
        </el-breadcrumb>
        <!-- <div class="title">启动度量日志</div> -->
      </div>
      <div class="wrap">
        <div class="search">
          <el-form :inline="true" label-position="right" label-width="100px" :model="formInline" ref="formInline" class="formInline" size="small" @submit.native.prevent>
            ...
            <el-form-item label="度量对象：" prop="object" v-show="advancedSearch==true">
              <el-input v-model="formInline.object" clearable placeholder="请输入度量对象"></el-input>
            </el-form-item>
        ...
        <div class="mainContainer">
          ...
          <el-table :data="startLogData" size="small" ref="startLogTable" tooltip-effect="light" 
            ...
            <el-table-column prop="object" label="度量对象" min-width="120%" show-overflow-tooltip></el-table-column>
            <el-table-column prop="hash" label="度量对象Hash" min-width="130%"></el-table-column>
            <el-table-column prop="stage" label="度量阶段" min-width="90%">
            ...
            <el-table-column prop="result" label="结果" min-width="80%">
              <template slot-scope="props">
                <span v-if="props.row.result == '1'">可信</span>
                <span v-else-if="props.row.result == '2'">不可信</span>
...
export default {
  ...
  data() {
    return {
      ...
      emptytxt:'',
      result:'',
      resultList:[{label:"所有结果",value:"0"},{label:"可信",value:"1"},{label:"不可信",value:"2"},{label:"未度量",value:"4"}],
      startLogData:[],
      currentPage: 1, // 当前页码
      total: 0, // 总条数
      pageSize: 10,// 每页的数据条数
  ...
  methods: {  
    loadData() {
      var that = this
      this.loading = true
      request({url: "/aud/log/bootLog",method: "post",
        data: {
    ...
    exportData() {
      ...
      this.$confirm("确定导出此数据？","提示", {
        ...
        request({url: "/aud/downloadLinuxBmeasure",method: "post",responseType: "blob", 
          data: form
        }).then((response) => {
          var res = response.data
          // console.log(res)
