    ...
    <div class="container">
      <div class="topNav">
        <el-breadcrumb class="breadNav" separator="/">
          ...
          <el-breadcrumb-item class="crumb2">可信连接日志</el-breadcrumb-item>
        </el-breadcrumb>
        <!-- <div class="title">可信连接日志</div> -->
      </div>
      <div class="wrap">
        <div class="search">
        ...
        <div class="mainContainer">
          ...
          <div class="popBtn">
            <el-button class="operateBtn" size="small" @click="exportData" :disabled="startLogData.length == 0">导出</el-button>
          </div>
          <el-table :data="startLogData" size="small" ref="startLogTable" tooltip-effect="light" 
            ...
            <el-table-column prop="peer_addr" label="对端IP地址" min-width="100%"></el-table-column>
            <el-table-column prop="local_session_id" label="本地会话ID" min-width="100%">
              <template slot-scope="props">
                <span v-if="props.row.action == '2' || props.row.local_session_id == '0'"></span>
                <span v-else>{{props.row.local_session_id}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="peer_session_id" label="对端会话ID" min-width="100%">
              <template slot-scope="props">
                <span v-if="props.row.action == '2' || props.row.peer_session_id == '0'"></span>
                <span v-else>{{props.row.peer_session_id}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="expire_time" label="会话过期时间" min-width="100%"></el-table-column>
            <el-table-column prop="is_bidir" label="双向会话" min-width="100%">
              <template slot-scope="props">
                <span v-if="props.row.action == '2'"></span>
                <span v-else-if="props.row.is_bidir == '0'">单向</span>
                <span v-else-if="props.row.is_bidir == '1'">双向</span>
              </template>
            </el-table-column>
            <el-table-column prop="action" label="动作" min-width="80%">
              <template slot-scope="props">
                <span v-if="props.row.action == '1'">协商成功</span>
                <span v-else-if="props.row.action == '2'">协商失败</span>
                <span v-else-if="props.row.action == '3'">创建会话</span>
                <span v-else-if="props.row.action == '4'">删除会话</span>
                <span v-else-if="props.row.action == '5'">会话过期</span>
                <span v-else-if="props.row.action == '6'">会话单向过期</span>
              </template>
            </el-table-column>
            <el-table-column prop="error_code" label="错误信息" min-width="100%">
              <template slot-scope="props">
                <span v-if="props.row.error_code == '8192'">内存不足</span>
                <span v-else-if="props.row.error_code == '8193'">初始化失败</span>
                <span v-else-if="props.row.error_code == '8194'">输入输出错误</span>
                <span v-else-if="props.row.error_code == '8195'">套接字操作失败</span>
                <span v-else-if="props.row.error_code == '8196'">非法协商数据流</span>
                <span v-else-if="props.row.error_code == '8197'">消息发送失败</span>
                <span v-else-if="props.row.error_code == '8198'">消息接收失败</span>
                <span v-else-if="props.row.error_code == '8199'">加密失败</span>
                <span v-else-if="props.row.error_code == '8200'">解密失败</span>
                <span v-else-if="props.row.error_code == '8201'">数据完整性验证失败</span>
                <span v-else-if="props.row.error_code == '8202'">防重放数据校验失败</span>
                <span v-else-if="props.row.error_code == '8203'">生成加密密钥失败</span>
                <span v-else-if="props.row.error_code == '8204'">生成可信凭证失败</span>
                <span v-else-if="props.row.error_code == '8205'">验证可信凭证失败</span>
                <span v-else-if="props.row.error_code == '8206'">三方仲裁验证失败</span>
                <span v-else-if="props.row.error_code == '8207'">获取可信连接策略失败</span>
                <span v-else-if="props.row.error_code == '8208'">获取远端IP失败</span>
                <span v-else-if="props.row.error_code == '8209'">获取本地IP失败</span>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {  
    loadData() {
      var that = this
      this.loading = true
      request({url: "/aud/log/trustLinkLog",method: "post",
        data: {
    ...
    exportData() {
      ...
      this.$confirm("确定导出此数据？","提示", {
        ...
        request({url: "/aud/downloadLinuxTrustLink",method: "post",responseType: "blob", data: form
        }).then((response) => {
          var res = response.data
          // console.log(res)
