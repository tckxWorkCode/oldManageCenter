    ...
    <div class="container">
      <div class="topNav">
        <el-breadcrumb class="breadNav" separator="/">
          ...
          <el-breadcrumb-item class="crumb2">白名单日志</el-breadcrumb-item>
        </el-breadcrumb>
        <!-- <div class="title">白名单日志</div> -->
      </div>
      <div class="wrap">
        <div class="search">
        ...
        <div class="mainContainer">
          ...
          <div class="popBtn">
            <el-button class="operateBtn" size="small" @click="exportData" :disabled="whiteListLogData.length==0">导出</el-button>
          </div>
          <el-table :data="whiteListLogData" size="small" ref="whiteListLogTable"  @sort-change='sortChange'
           ...
           :cell-style="{color: 'rgba(0, 0, 0, 0.9)'}" tooltip-effect="light" style="width: 99.9%;font-size: 14px"   :fit='true'>
            ...
            <el-table-column prop="hash" label="客体Hash" min-width="130%">
              <template slot-scope="props">
            ...
            <el-table-column prop="result" label="结果" min-width="60%">
              <template slot-scope="props">
                ...
                <!-- <span v-else-if="props.row.result == '3'">学习</span> -->
                <span v-else-if="props.row.result == '4'">未度量</span>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {  
    loadData() {
      var that = this
      this.loading = true
      request({url: "/aud/log/whiteLog",method: "post",
        data: {
        ...
        if (res.list) {
          ...
          that.whiteListLogData = res.list
        }
    ...
    exportData() {
      ...
      this.$confirm("确定导出此数据？","提示", {
        ...
        request({url: "/aud/downloadLinuxWhitelist",method: "post",responseType: "blob", 
          data: form
        }).then((response) => {
          var res = response.data
          // console.log(res)
