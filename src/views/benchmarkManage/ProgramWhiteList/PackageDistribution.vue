<template>
  <div class="main">
    <div class="container">
      <div class="topNav">
        <el-breadcrumb class="breadNav" separator="/">
          ...
          <el-breadcrumb-item class="crumb2">程序包下发</el-breadcrumb-item>
        </el-breadcrumb>
        <!-- <div class="title">程序包下发</div> -->
      </div>
      <div class="wrap">
        <div class="search">
        ...
        <div class="mainContainer">
          <div class="tableTitle">程序包下发</div>
          <div class="popBtn">
          ...
          <el-table :data="issueData" size="small" ref="issueTable" tooltip-effect="light" style="width: 99.9%;font-size: 14px"   
            ...
            :cell-style="{color: 'rgba(0, 0, 0, 0.9)',padding:'0px'}" @sort-change='sortChange' :fit='true' row-key="policyId" :expand-row-keys="expands"  @expand-change="expandrow">
            <template slot="empty">
              <div>暂无数据</div>
            </template>
            ...
            <el-table-column prop="ip" label="网络地址" min-width="130%"></el-table-column>
            ...
            <el-table-column prop="packageName" label="程序包名称" min-width="100%"></el-table-column>
            <el-table-column prop="policyName" label="策略名称" min-width="100%"></el-table-column>
            ...
            <el-table-column prop="status" label="状态" min-width="100%">
              <template slot-scope="props">
                <span v-if="props.row.status == '0'">等待更新</span>
                <span v-else-if="props.row.status == '1'">已更新</span>
                <span v-else-if="props.row.status == '2'">等待停用</span>
                <span v-else-if="props.row.status == '3'">已停用</span>
                <span v-else-if="props.row.status == '4'">下发失败</span>
              </template>
            </el-table-column>
            <el-table-column prop="createDate" label="创建时间" sortable='custom' min-width="100%" show-overflow-tooltip></el-table-column>
                ...
                <div class="decoration">
                  <div :id="props.row.policyId" style="border-bottom:1px solid #DDDEE0">
                    <el-table :data="expandData" border width="100%" :header-cell-style="headClass" :cell-style="rowClass">
                      ...
                      <el-table-column prop="sPackconName" label="文件名" min-width="100%"></el-table-column>
                      ...
                      <el-table-column prop="upDate" label="时间" min-width="100%"></el-table-column>
                    </el-table>
                  </div>
            ...
            <el-table-column fixed="right" label="操作" align="left" width="150px">
              <template slot-scope="props">
                <el-button style="color:#1ac492" v-if="props.row.status == '3'" @click="startfun(props.row)" type="text">启用</el-button>
                <el-button v-else disabled type="text">启用</el-button>
                <el-button style="color:#1ac492" v-if="props.row.status == '1'"  @click="stopUsing(props.row)" type="text">停用</el-button>
    ...
    <issue-win ref="windows" @newsuccess="newsuccess"></issue-win>
  </div>
</template>
<script>
...
import IssueWin from './pop/IssueWin.vue'
export default {
  name:"PackageDistribution",
  components: { IssuePop,IssueWin },
  data() {
    return {
      loading:false,
      formInline:{
  ...
  methods: {  
    loadData() {
      var that = this
      this.loading = true
      request({url: "/sec/whiteListLearnPolicySearch",method: "post",
        data: {
    ...
    expandrow (row, expandedRows) {
      ...
      // console.log(row)
      request({
        url: '/sec/whiteListPolicyContentSearch',method: 'post',
        data: {
          condition: '',
          orderBy: '',
