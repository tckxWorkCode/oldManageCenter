  ...
  <el-dialog title="新建策略" width="32%" top="5vh" :visible.sync="dialogFormVisible" @close="diaClose" :close-on-click-modal="false">
    <el-form :label-position="labelPosition" label-width="120px" :rules="rules" :model="ruleForm" ref="ruleForm" size="small">
      ...
      <el-form-item label="系统类型：" prop="systemType" >
        <el-select v-model="ruleForm.systemType" placeholder="请选择系统类型" @change="currentSelect">
          <el-option label="Linux" :value="1"></el-option>
          <el-option label="window" :value="2"></el-option>
        </el-select>
      </el-form-item>
      <div class="itemw">
      ...
      that.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (that.ruleForm.systemType == "1") {
            ...
            request({ url:"/sec/whiteListIssuedByWin", method: "post",data:JSON.stringify(data)}).then((response) => {
              var res = response.data
              // console.log(res)
              if (res.status == 500) {
                that.btnvalue1 = false;
    ...
    loadNList() {// 加载终端列表
      ...
      request({url: "/LNodeList",method: "post",
        data: {
          ...
          system_type : this.ruleForm.systemType,
          online_status: 1,
          searchPage: "1",
          searchSize: "10000",
        }
