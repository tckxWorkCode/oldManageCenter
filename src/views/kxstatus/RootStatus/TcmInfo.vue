    ...
    <div class="container">
      <div class="topNav">
        <el-breadcrumb class="breadNav" separator="/">
          ...
          <el-breadcrumb-item class="crumb2">TCM管理</el-breadcrumb-item>
        </el-breadcrumb>
        <!-- <div class="title">TCM管理</div> -->
      </div>
      <div class="wrap">
        <div class="search">
        ...
        <div class="mainContainer">
          <div class="tableTitle">TCM管理</div>
          <el-table :data="tcmData" size="small" ref="tcmTable" tooltip-effect="light" :cell-style="{color: 'rgba(0, 0, 0, 0.9)'}" 
            ...
            <el-table-column prop="nodeName" label="终端名称" min-width="130%" :show-overflow-tooltip="true"></el-table-column>
            <el-table-column prop="nodeIp" label="网络地址"></el-table-column>
            <el-table-column prop="beHostTime" label="获取时间" :show-overflow-tooltip="true" sortable='custom'></el-table-column>
            <el-table-column prop="beEkGenerated" label="EK生成">
              <template slot-scope="props">
                <span v-if="props.row.beEkGenerated == '0'">未生成</span>
                <span v-else>已生成</span>
              </template>
            </el-table-column>
            <el-table-column prop="beSrkGenerated" label="SMK生成">
              <template slot-scope="props">
                <span v-if="props.row.beSrkGenerated == '0'">未生成</span>
                <span v-else>已生成</span>
              </template>
            </el-table-column>
            <el-table-column prop="bePikGenerated" label="PIK生成">
              <template slot-scope="props">
                <span v-if="props.row.bePikGenerated == '0'">未生成</span>
                <span v-else>已生成</span>
              </template>
            </el-table-column>
            <el-table-column prop="bePesistentKeyNumber" label="持久存储密钥数量" min-width="100%"></el-table-column>
            <el-table-column prop="beAlgMode" label="算法实现" min-width="80%">
              <template slot-scope="props">
                <span v-if="props.row.beAlgMode == '1'">软算法</span>
                <span v-else-if="props.row.beAlgMode == '2'">硬算法</span>
              </template>
            </el-table-column>
          </el-table>
