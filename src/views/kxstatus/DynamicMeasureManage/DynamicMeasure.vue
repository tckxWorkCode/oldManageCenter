      ...
      <div class="wrap">
        ...
        <div class="mainContainer">
          ...
          <el-table :data="dynamicMeasureData" size="small" ref="dynamicMeasureTable" tooltip-effect="light" 
            ...
            :header-cell-style="{height: '48px','background-color': '#F0F2F5',color: 'rgba(0, 0, 0, 0.72)',padding:'3px 0','text-align':'center'}"
            @sort-change='sortChange' :cell-style="{color: 'rgba(0, 0, 0, 0.9)','text-align':'center'}" style="width: 99%;font-size: 14px"   :fit='true' 
            ...
            <el-table-column prop="ip" label="网络地址" min-width="100%" show-overflow-tooltip>
            ...
            <el-table-column prop="system" label="系统类型" show-overflow-tooltip>
              <template slot-scope="scope">
                <span v-if="scope.row.system == 'Linux' || scope.row.system == 'linux'|| scope.row.system == 'Unix'|| scope.row.system == 'unix'">
                  <img src="@/assets/linux_icon.png"><span style="padding-left:10px">{{scope.row.system}}</span>
                </span>
                <span v-else-if="scope.row.system && scope.row.system.indexOf('win')">
                  <img src="@/assets/windows_icon.png"> <span>{{scope.row.system}}</span>
                </span>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="systemCallSwitch">
              <template slot="header">
                <div class="nowrap">系统调用表</div>
                <div>开关</div>
              </template>
              <template slot-scope="scope">
                <span v-if="scope.row.systemCallSwitch == '1'">开启</span>
                <span v-else-if="scope.row.systemCallSwitch == '0'">关闭</span>
              </template>
            </el-table-column>
            <el-table-column prop="kernalCodeSegSwitch">
              <template slot="header">
                <div class="nowrap">内核代码段</div>
                <div>开关</div>
              </template>
              <template slot-scope="scope">
                <span v-if="scope.row.kernalCodeSegSwitch == '1'">开启</span>
                <span v-else-if="scope.row.kernalCodeSegSwitch == '0'">关闭</span>
              </template>
            </el-table-column>
            <el-table-column prop="dmIdtSwitch" label="中断描述符开关">
              <template slot="header">
                <div class="nowrap">中断描述符</div>
                <div>开关</div>
              </template>
              <template slot-scope="scope">
                <span v-if="scope.row.dmIdtSwitch == '1'">开启</span>
                <span v-else-if="scope.row.dmIdtSwitch == '0'">关闭</span>
              </template>
            </el-table-column>
            <el-table-column prop="moduleListSwitch" label="模块列表开关">
              <template slot="header">
                <div class="nowrap">模块列表</div>
                <div>开关</div>
              </template>
              <template slot-scope="scope">
                <span v-if="scope.row.moduleListSwitch == '1'">开启</span>
                <span v-else-if="scope.row.moduleListSwitch == '0'">关闭</span>
              </template>
            </el-table-column>
            <el-table-column prop="filesystemSwitch" label="文件系统开关">
              <template slot="header">
                <div class="nowrap">文件系统</div>
                <div>开关</div>
              </template>
              <template slot-scope="scope">
                <span v-if="scope.row.filesystemSwitch == '1'">开启</span>
                <span v-else-if="scope.row.filesystemSwitch == '0'">关闭</span>
              </template>
            </el-table-column>
            <el-table-column prop="networkSwitch" label="网络协议开关">
              <template slot="header">
                <div class="nowrap">网络协议</div>
                <div>开关</div>
              </template>
              <template slot-scope="scope">
                <span v-if="scope.row.networkSwitch == '1'">开启</span>
                <span v-else-if="scope.row.networkSwitch == '0'">关闭</span>
              </template>
            </el-table-column>
            <el-table-column prop="gdtSwitch" label="全局描述符开关">
              <template slot="header">
                <div class="nowrap">全局描述符</div>
                <div>开关</div>
              </template>
              <template slot-scope="scope">
                <span v-if="scope.row.gdtSwitch == '1'">开启</span>
                <span v-else-if="scope.row.gdtSwitch == '0'">关闭</span>
              </template>
            </el-table-column>
            <el-table-column prop="status" label="状态" :formatter="statusVal" min-width="100%" show-overflow-tooltip></el-table-column>
            <el-table-column prop="updateDate" label="更新时间" sortable='custom' min-width="100%" show-overflow-tooltip></el-table-column>
          </el-table>
