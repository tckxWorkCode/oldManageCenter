<template>
  <!--导入安装包开始-->
  <el-dialog title="导入证书" width="30%" :visible.sync="dialogFormVisible" @close="diaclose" :close-on-click-modal="false" class="dialogw1">
    <el-form :label-position="labelPosition" :rules="rules" :model="ruleForm" ref="ruleForm" size="small">
      <el-form-item class="uploadItem" prop="other_name" label="证书别名：">
        <el-input style="width: 80%;" v-model="ruleForm.other_name" size="small" placeholder="请输入证书别名" maxlength="255" @change="handleOthername"></el-input>
      </el-form-item>
      <el-form-item label="" prop="file" class="uploadfilec">
        <el-upload class="uploadfilediv" :http-request="uploadFile" :show-file-list="true"
          name="file" ref="upload" action="url"
        ...
        <div v-if="this.ruleForm.file" style="float:left;width:80%;border:1px solid #dfdfdf;padding:15px 20px;margin:15px 25px">
          ...
          <div style="float:left;text-align:left;line-height:20px">
            ...
            <div class="remindtxt2">{{this.ruleForm.size}} 字节</div>
          </div>
...
export default {
  ...
  data() {
    var validatePass = (rule, value, callback) => {
      if (value === "") {
        callback(new Error("请输入证书别名"));
      } else if (value.length > 20) {
        callback(new Error("证书别名名称长度不超出20个字符"));
      } /*else if (/^\s+$/gi.test(value)) {
        callback(new Error('证书别名不能为空格'))
      } */ else if (
        /[^\a-\z\A-\Z0-9\u4E00-\u9FA5\_\.]/g.test(value)
      ) {
        callback(new Error("证书别名只能输入中文、英文、数字、_和.符号"));
      } else {
        callback();
      }
    };
    return {
      ...
      rules: {
        ...
        file: [{ required: true, message: "请选择证书", trigger: "change" }],
      },
      ruleForm: {
        other_name: "",
        file: "",
  ...
  methods: {
        ...
        if (valid) {
          if ( that.aliaspro.indexOf(that.ruleForm.other_name) === -1 && that.importfileName.indexOf(that.ruleForm.file) === -1) {
            ...
            if (that.aliaspro.indexOf(that.ruleForm.other_name) !== -1) {
              that.$message({ message: "证书别名重复,请检查!!!", type: "error", showClose: true, });
            } else if ( that.importfileName.indexOf(that.ruleForm.file) !== -1) {
              that.$message({ message: "证书名称重复,请检查!!!", type: "error", showClose: true, });
            }
          }
        } else {
    ...
    diaOpen(param, param1) {
      ...
      that.url = "/sys/lCertSecondUpload";
    ...
    uploadFile(content) {
      console.info("123213123213", content);
      var that = this;
      that.$message.closeAll();
    ...
    //   } else {
        ...
        that.uploadmessage = "";
        that.ruleForm.file = file.name;
        that.ruleForm.size = (file.size).toFixed(2);
