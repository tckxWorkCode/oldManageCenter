    ...
    <div class="container">
      <div class="topNav">
        <el-breadcrumb class="breadNav" separator="/">
          ...
          <el-breadcrumb-item class="crumb2">证书导入</el-breadcrumb-item>
        </el-breadcrumb>
        <!-- <div class="title">证书导入</div> -->
      </div>
      <div class="wrap">
        <div class="search">
          <el-form :inline="true" label-position="right" label-width="145px" :model="formInline" ref="formInline" class="formInline" size="small" @submit.native.prevent>
            <el-form-item label="证书名称/证书别名：" prop="terminalName">
              <el-input v-model="formInline.terminalName" clearable placeholder="请输入证书名称/证书别名" oninput="value=value.replace(/[%]/g, '')"></el-input>
            </el-form-item>
        ...
        <div class="mainContainer">
          <div class="tableTitle">导入证书列表</div>
          <div class="popBtn">
          ...
          <el-table :data="softwareData" size="mini" ref="softwareTable" tooltip-effect="light" style="width: 99.9%;font-size: 14px"   
            ...
            :header-cell-style="{height: '48px','background-color': '#F0F2F5',color: 'rgba(0, 0, 0, 0.72)',padding:'0px'}"
              @sort-change='sortChange' :cell-style="{padding:'0px',color: 'rgba(0, 0, 0, 0.9)'}" :fit='true' row-key="certi_id" class="softwareTable">
            <template slot="empty">
              <div>暂无数据</div>
            </template>
            <el-table-column type="index" :index="indexMethod" label="序号" width="70px"></el-table-column>
            <el-table-column prop="certi_name" label="证书名称" min-width="100%" show-overflow-tooltip></el-table-column>
            <el-table-column prop="orther_name" label="证书别名"></el-table-column>
            <el-table-column prop="certi_path" label="证书存放路径" min-width="200%"></el-table-column>
            <el-table-column prop="status" label="证书状态">
              <template slot-scope="props">
                <span v-if="props.row.status == '0'||props.row.status == ''">未使用</span>
                <span v-else-if="props.row.status == '1'">已使用</span>
              </template>
            </el-table-column>
            <el-table-column prop="create_time" label="创建时间" sortable='custom' min-width="100%" show-overflow-tooltip></el-table-column>
            <el-table-column prop="start_time" label="有效期始" show-overflow-tooltip></el-table-column>
            <el-table-column prop="end_time" label="有效期止" show-overflow-tooltip></el-table-column>
            <el-table-column fixed="right" label="操作" header-align="left" width="80px">
              <template slot-scope="scope">
                <el-button type="text" style="color:#FF4D4F" :id="scope.row.certi_id" @click="del(scope.row.certi_id)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
  ...
  methods: {  
    ...
    del(id) {
      ...
      this.$confirm("此操作将永久删除该数据，是否继续？","提示", {
        ...
        request({url: "/sys/deletelCertSecond?cert_id=" + cert_id,method: "post",
        }).then((response) => {
          var res = response.data
          // console.log(res)
          if(res.status==0) {
